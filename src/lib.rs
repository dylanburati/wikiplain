...
impl<'a> From<Token<'a>> for PyToken {
    fn from(value: Token<'a>) -> Self {
        match value {
            ...
            Token::ElementStart(name, true) => PyToken::new(PyTokenKind::ElementStartEnd, Some(name)),
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.kind {
            ...
            PyNodeKind::Element => {
                ...
                f.debug_tuple("Element").field(&name).field(&self.children).finish()
            },
...
impl From<Node> for PyNode {
    fn from(value: Node) -> Self {
        match value {
            Node::Document(children) => PyNode::new(PyNodeKind::Document, children.iter().cloned().map(PyNode::from).collect(), None),
            Node::Element(name, children) => PyNode::new(PyNodeKind::Element, children.iter().cloned().map(PyNode::from).collect(), Some(name)),
            Node::Template(children) => PyNode::new(PyNodeKind::Template, children.iter().cloned().map(PyNode::from).collect(), None),
            Node::Link(children) => PyNode::new(PyNodeKind::Link, children.iter().cloned().map(PyNode::from).collect(), None),
            Node::Argument(children) => PyNode::new(PyNodeKind::Argument, children.iter().cloned().map(PyNode::from).collect(), None),
...
fn parse(text: &str) -> PyResult<PyNode> {
    wikitext_ast::parse_document(text)
        .map_err(|err| PyValueError::new_err(err.to_string()))
        .map(|doc| doc.into())
}
